...
object ModFluids {
    ...
    fun init() {
        STILL_MANA = Registry.register<Fluid, ManaFluid.Still>(Registry.FLUID, Wizardry.getId("mana"), ManaFluid.Still())
        FLOWING_MANA = Registry.register<Fluid, ManaFluid.Flowing>(Registry.FLUID, Wizardry.getId("flowing_mana"), ManaFluid.Flowing())
        ModItems.manaBucket = BucketItem(STILL_MANA, Item.Settings().recipeRemainder(Items.BUCKET).maxCount(1))

        STILL_NACRE = Registry.register<Fluid, NacreFluid.Still>(Registry.FLUID, Wizardry.getId("nacre"), NacreFluid.Still())
        FLOWING_NACRE = Registry.register<Fluid, NacreFluid.Flowing>(Registry.FLUID, Wizardry.getId("flowing_nacre"), NacreFluid.Flowing())
