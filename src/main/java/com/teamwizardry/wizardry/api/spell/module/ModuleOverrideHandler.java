		...
		for (Entry<String, OverrideMethod> entry : overrides.entrySet()) {
			...
			if (ptr == null) {
				...
				if (!areMethodsCompatible(ptr.getBaseMethod().getMethod(), entry.getValue().getMethod()))
					throw new ModuleOverrideException("Method '" + ptr.getClass().getName() + "." + ptr.getBaseMethod().getMethod().getName() + "' can't be overridden by '" + entry.getValue().getClass().getName() + "." + entry.getValue().getMethod().getName() + "' due to incompatible signature.");
