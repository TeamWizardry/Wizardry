...
public class WizardryStructure extends Structure {
	...
	public HashMultimap<BlockRenderLayer, Template.BlockInfo> blocks = HashMultimap.create();
	@SideOnly(Side.CLIENT)
	public EnumMap<BlockRenderLayer, int[]> vboCaches = new EnumMap<>(BlockRenderLayer.class);

	private boolean builtVBO = false;
	private IBlockAccess access = null;
	private Vec3i offset = Vec3i.NULL_VECTOR;

	public WizardryStructure(@Nonnull ResourceLocation loc) {
		super(loc);
	}

	public WizardryStructure(@Nonnull ResourceLocation loc, Vec3i offset) {
		super(loc);
		this.offset = offset;
	}
