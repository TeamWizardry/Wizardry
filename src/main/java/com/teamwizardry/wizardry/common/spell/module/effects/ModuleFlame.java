        ...
        if (caster instanceof EntityItem) {
			...
			if (result != null) {
				if (duration >= item.getEntityItem().stackSize) {
                    EntityItem output = new EntityItem(item.world, item.posX, item.posY + 0.5, item.posZ);
                    result.stackSize *= item.getEntityItem().stackSize;
					output.setEntityItemStack(result);
                    output.world.spawnEntity(output);
                    item.setDead();
				} else {
                    EntityItem output = new EntityItem(item.world, item.posX, item.posY + 0.5, item.posZ);
			...
			int duration = spell.getInteger(Constants.Module.DURATION);
            caster.setFire(MathHelper.ceil(duration / 20.0));
			...
			if (result != null) {
                ...
                if (!caster.world.isRemote) {
                    caster.world.setBlockState(pos, smelted.getStateFromMeta(result.getMetadata()));
                    caster.world.playEvent(2001, pos, Block.getStateId(smelted.getDefaultState()));
                }
			}
		}

        LibParticles.EFFECT_FIRE(caster.world, caster.getPositionVector().addVector(0.0, 1.0, 0.0), Vec3d.ZERO, 0.7f);
