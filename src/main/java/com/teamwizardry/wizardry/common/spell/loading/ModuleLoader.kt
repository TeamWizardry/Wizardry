...
//     *         stream
//     */
////    public static List<Module> loadModules(InputStream file, Function<Identifier, Pattern> patternSupplier, Function<Identifier, Item> itemSupplier)
////    {
...
////    private static Module compileModule(Map<String, Object> yaml, Function<Identifier, Pattern> patternSupplier, Function<Identifier, Item> itemSupplier)
////    {
////        // Straightforward components
////        Pattern pattern = patternSupplier.apply(new Identifier((String) yaml.get(MODULE)));
////        String name = (String) yaml.get(NAME);
////        List<Item> items = ((List<String>) yaml.get(ITEMS)).stream().map(Identifier::new).map(itemSupplier::apply).collect(Collectors.toList());
