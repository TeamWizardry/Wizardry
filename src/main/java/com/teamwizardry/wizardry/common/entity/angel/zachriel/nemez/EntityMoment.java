...
public final class EntityMoment {
    ...
    public void apply(Entity entity, EntityMoment nextMoment, float partialTicks) {
        ...
        if (pitch != null) entity.rotationPitch = pitch + (nextMoment.pitch != null ? (nextMoment.pitch - pitch) * partialTicks : 0);
        if (entity instanceof EntityLivingBase) {
            EntityLivingBase living = (EntityLivingBase) entity;
            if (health != null) living.setHealth(health + (nextMoment.health != null ? (nextMoment.health - health) * partialTicks : 0));
            if (entity instanceof EntityPlayer) {
                EntityPlayer player = (EntityPlayer) living;
                if (food != null) player.getFoodStats().setFoodLevel(food + (int) (nextMoment.food != null ? (nextMoment.food - food) * partialTicks : 0));
                if (saturation != null) saturationSetter.invoke(player.getFoodStats(), saturation + (nextMoment.saturation != null ? (nextMoment.saturation - saturation) * partialTicks : 0));
                if (exhaustion != null) exhaustionSetter.invoke(player.getFoodStats(), exhaustion + (nextMoment.exhaustion != null ? (nextMoment.exhaustion - exhaustion) * partialTicks : 0));
            }
        }
    }
    ...
    public boolean matches(Entity entity) {
        ...
        if (entity instanceof EntityLivingBase) {
            ...
            if (entity instanceof EntityPlayer) {
                ...
                if (saturation != null && player.getFoodStats().getSaturationLevel() != saturation) return false;
            }
        }
