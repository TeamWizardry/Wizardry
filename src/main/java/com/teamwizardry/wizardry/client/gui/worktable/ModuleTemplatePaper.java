...
public class ModuleTemplatePaper extends ModuleTemplate {
	...
	public ModuleTemplatePaper(int posX, int posY, Module constructor, GuiComponent<?> paper) {
		...
		mouseLine.endPos.func((Function<ComponentModuleLine, Vec2d>) ComponentModuleLine::getMousePosThisFrame);
		...
		getResult().BUS.hook(GuiComponent.MouseUpEvent.class, (event) -> {
			if (event.getButton() == EnumMouseButton.RIGHT) {
				if (event.getComponent().getMouseOver() && (data.draggingFrom != this)) {
					if (connections.contains(data.draggingFrom)) {
						connections.remove(data.draggingFrom);
						lines.removeByTag(data.draggingFrom);
						...
						line.endPos.func((Function<ComponentModuleLine, Vec2d>) (_c_) -> {
							if (getResult().getParent() == null) {
								_c_.invalidate();
								module.connections.remove(this);
							}
							return getResult().getPos().sub(module.getResult().getPos());
						});
						module.lines.add(line);
					}
				}
				if ((data != null) && (data.draggingFrom == this)) {
					mouseLine.setVisible(false);
				}
			}
		});
